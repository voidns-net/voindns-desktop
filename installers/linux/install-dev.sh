...
echo "[$APP] building service (release)…"
cargo build --release -p voidns-service --manifest-path "$ROOT/Cargo.toml"

echo "[$APP] installing service (root)…"
...
echo
echo "[$APP] done. Now run the GUI WITHOUT root:  ./crates/gui/run.sh"

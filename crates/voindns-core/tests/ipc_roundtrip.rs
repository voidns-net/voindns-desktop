...
use voindns_core::{ipc, Controller};
use voindns_proto::{Command, ConnState, Event};
...
async fn ipc_ping_status_subscribe() {
    ...
    let sock = if cfg!(windows) {
        format!("voindns-test-{}", std::process::id())
    } else {
        format!("/tmp/voindns-test-{}.sock", std::process::id())
    };
    std::env::set_var("VOINDNS_SOCK", &sock);

...
use voindns_proto::{Command, Event, Status};

use crate::Controller;

/// Resolve the IPC endpoint name. `VOINDNS_SOCK` overrides the default (handy
/// for unprivileged dev/tests).
fn endpoint_name() -> io::Result<Name<'static>> {
    let name: &'static str = match std::env::var("VOINDNS_SOCK") {
...
fn default_name() -> &'static str {
    #[cfg(windows)]
    {
        voindns_proto::IPC_PIPE_NAME
    }
    #[cfg(not(windows))]
    {
        voindns_proto::IPC_SOCK_PATH
    }
}
...
fn prepare_unix_path() {
    use std::path::Path;
    let path = match std::env::var("VOINDNS_SOCK") {
        Ok(s) => s,
        Err(_) => voindns_proto::IPC_SOCK_PATH.to_string(),
    };
...
fn relax_socket_perms() {
    ...
    let path =
        std::env::var("VOINDNS_SOCK").unwrap_or_else(|_| voindns_proto::IPC_SOCK_PATH.to_string());

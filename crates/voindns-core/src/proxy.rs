...
// --- voindns hosted DoH endpoint (placeholder bootstrap until provisioned) ---
// The IP is hardcoded so the proxy never recursively resolves its own upstream
// (RFC 8484 §10 bootstrap problem). Replace with the real voindns anycast IP.
const VOINDNS_DOH_IP: &str = "1.1.1.1";
const VOINDNS_DOH_HOST: &str = "cloudflare-dns.com";
const VOINDNS_DOH_PATH: &str = "/dns-query";
...
fn upstream_name_servers(upstream: &UpstreamSel) -> Result<Vec<NameServerConfig>> {
    let servers = match upstream {
        ...
        UpstreamSel::Voindns => {
            let ip: IpAddr = VOINDNS_DOH_IP.parse().context("voindns DoH IP")?;
            vec![NameServerConfig::https(
                ip,
                Arc::from(VOINDNS_DOH_HOST),
                Some(Arc::from(VOINDNS_DOH_PATH)),
            )]
        }

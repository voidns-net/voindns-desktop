...
//! `voindns-service`. Both sides serialize these types as newline-free,
//! length-delimited JSON frames (see `voindns-core::ipc`).
...
/// (`\\.\pipe\voindns-service`); on Unix a filesystem socket path.
pub const IPC_PIPE_NAME: &str = "voindns-service";
pub const IPC_SOCK_PATH: &str = "/run/voindns/control.sock";

/// Upstream DoH resolver selection.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(tag = "kind", rename_all = "snake_case")]
pub enum UpstreamSel {
    /// The voindns hosted DoH endpoint. Carries a bootstrap IP so the proxy
